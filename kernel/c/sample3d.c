...
void sample3d_overlay (int cell)
{
  ...
  // highlight cell
  ezv_set_cpu_color_1D (ctx [0], cell, 1, ezv_rgb (255, 0, 0));
}
...
void sample3d_draw_uni (void)
{
  draw_uniform (0.5f);
}

void sample3d_draw_linear (void)
{
  for (int c = 0; c < NB_CELLS; c++)
    cur_data (c) = c / (float)(NB_CELLS - 1);
}

void sample3d_draw (char *param)
{
  if (param) {
  ...
  hooks_draw_helper (param, sample3d_draw_uni);
}
