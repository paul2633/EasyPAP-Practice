...
void mesh_data_refresh (unsigned iter)
{
  ...
  if (picking_enabled) {
    ...
    if (picked_cell != -1) {
      ...
      ezv_hud_set (ctx [0], cell_hud, "Cell: %d", picked_cell);

      int partoche =
          mesh3d_obj_get_patch_of_cell (&easypap_mesh_desc, picked_cell);

      ezv_hud_on (ctx [0], partition_hud);
      ezv_hud_set (ctx [0], partition_hud, "Patch: %d", partoche);

      if (the_1d_overlay != NULL)
        the_1d_overlay (picked_cell);
      else {
        // partition
        ...
        // 0xC0));
        // cell
        ezv_set_cpu_color_1D (ctx [0], picked_cell, 1,
...
{
  picked_cell = ezv_perform_1D_picking (ctx, 1);
}
