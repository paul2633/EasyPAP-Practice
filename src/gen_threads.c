...
static unsigned nb_threads      = 0;
static unsigned iterations      = 0;
static unsigned done_iterations = 0;
...
static unsigned current_change         = 0;
static unsigned previous_change        = 0;
static gen_tile_func_t gen_tile        = do_tile_id;
static gen_patch_func_t gen_patch      = do_patch_id;
...
static unsigned end_of_iteration (unsigned me, unsigned change)
{
  if (change)
    current_change = 1;
...
static void *thread_2D_block (void *arg)
{
  ...
  for (unsigned it = 1; it <= iterations; it++) {
    ...
    for (unsigned tile = first; tile < first + count; tile++) {
      ...
      if (gen_tile (x, y, TILE_W, TILE_H, me))
        change = 1;
    }

    if (end_of_iteration (me, change) == 0)
      break;
  }
...
static void *thread_3D_cyclic (void *arg)
{
  ...
  for (unsigned it = 1; it <= iterations; it++) {
    ...
    for (unsigned firstp = me * chunk_size; firstp < NB_PATCHES;
      ...
      for (unsigned p = firstp; p < MIN (firstp + chunk_size, NB_PATCHES);
           p++) {
        if (gen_patch (p, me))
          change = 1;
      }
...
void gen_threads_config_2D (gen_threads_schedule_t sched, trail_func_t trail, gen_tile_func_t tile_func)
{
  ez_pthread_init ();
  ...
  if (schedule == GEN_THREADS_SCHED_CYCLIC ||
    ...
    pthread_distrib_init (&distrib, nb_threads, nb_elements, NULL);
  }
}

void gen_threads_config_3D (gen_threads_schedule_t sched, trail_func_t trail, gen_patch_func_t patch_func)
{
  ez_pthread_init ();
